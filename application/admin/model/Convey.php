...
class Convey extends Model
{
    ...
    public function create_order($uid,$cid=1)
    {
        ...
        //判断是否连单
        $setorder = Db::name('xy_users_setorder')->where(['uid'=>$uid])->find();
        $have_order = Db::name('xy_convey')->where(['uid'=>$uid])->whereDay('addtime')->count();
        $next_order = $have_order+1;

        if (!empty($setorder) && $next_order==$setorder['order_sort']){
            //查询商品 计算商品
            $price = Db::name('xy_goods_list')->field('goods_price')->where(['id'=>$setorder['goods_id']])->find();
            $goods['num'] = $price['goods_price']*$setorder['goods_num'];
            $goods['count'] = $setorder['goods_num'];
            $goods['id'] = $setorder['goods_id'];
            $goods['set_id'] = $setorder['id'];
        }else{
        ...
        //连单佣金设置
        $ulevel['bili'] = $setorder['goods_com']?$setorder['goods_com']:$ulevel['bili'];
        ...
        $res1 = Db::name($this->table)
                ->insert([
                    ...
                    'set_id' => isset($goods['set_id'])?$goods['set_id']:0
                ]);
        if($res && $res1){
            Db::commit();
