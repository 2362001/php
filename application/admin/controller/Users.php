...
//            $show_tel2 = input('post.show_tel2/d',0);  //显示电话隐藏
    ...
    public function edit_users_order_setting(){
        $id = input('get.id',0);
        if(!$id) $this->error('参数错误');
        $this->today_order = Db::name('xy_convey')->whereDay('addtime')->where(['uid'=>$id])->count();
        if(request()->isPost()){
            $set_order    = input('post.set_order/d',0);
            $set_goods  = input('post.set_goods/s','');
            if(!$set_order) $this->error('请设置连单单号！');
            if(!$set_goods) $this->error('请选择连单商品！');
            //$order = Db::name('xy_convey')->where(['uid'=>$id])->count();
            //if ($set_order<=$this->today_order) $this->error('该用户已经抢到'.$this->today_order.'单');
            //整理入库数据
            ...
            //$data['status'] = 0;
            $data['edit_time'] = time();
            $setinfo = Db::name('xy_users_setorder')->where(['uid'=>$id])->find();
            if ($setinfo){
                Db::name('xy_users_setorder')->where(['uid'=>$id])->update($data);
            }else{
                $data['uid'] = $id;
                $data['add_time'] = time();
        ...
        $this->info = Db::table($this->table)->find($id);
        $this->cate = Db::name('xy_goods_cate')->select();
        $setorder = Db::name('xy_users_setorder')->where(['uid'=>$id])->find();
        if(!empty($setorder)){
            $this->setorder = '第'.$setorder['order_sort'].'单-[商品'.$setorder['goods_id'].'X'.$setorder['goods_num'].'X'.$setorder['goods_com'].']';
        }else{
            $this->setorder = '暂未设置派单';
        }
        return $this->fetch();
    }
    //获取区间商品
    public function get_goods(){
        ...
        return json(['code'=>401,'msg'=>'查无数据！']);
    }
    /**
     * 连单管理
     */
    public function edit_users_order_manage(){
        $id = input('get.id',0);
        if(!$id) $this->error('参数错误');
        $this->info = Db::table($this->table)->find($id);
        $this->order = Db::name('xy_users_setorder')->where(['uid'=>$id])->find();
        if (!empty($this->order)){
            $order = Db::name('xy_convey')->where(['uid'=>$id,'status'=>0,'set_id'=>$this->order['id']])->find();
            if ($order){
                $goods = Db::name('xy_goods_list')->field('goods_name,goods_price,goods_pic')->where(['id'=>$order['goods_id']])->find();
                if (!empty($goods)){
                    $this->order['goods_name'] = $goods['goods_name'];
                    $this->order['goods_price'] = $goods['goods_price'];
                    $this->order['goods_pic'] = $goods['goods_pic'];
                }
            }else{
                $this->order = [];
            }
    ...
    public function edit_goods(){
        ...
        $set = Db::name('xy_users_setorder')->where(['id'=>$setid])->find();
        $price = Db::name('xy_goods_list')->field('goods_price')->where(['id'=>$id])->find();
        $goods['num'] = $price['goods_price']*$num;
        $goods['goods_count'] = $num;
        $goods['goods_id'] = $id;
        $uinfo = Db::name('xy_users')->field('level')->find($set['uid']);
        $level = $uinfo['level'];
        !$uinfo['level'] ? $level = 0 : '';
        ...
        $goods['commission'] = $goods['num']*$ulevel['bili'];
        Db::name('xy_convey')->where(['uid'=>$set['uid'],'status'=>0,'set_id'=>$setid])->update($goods);
